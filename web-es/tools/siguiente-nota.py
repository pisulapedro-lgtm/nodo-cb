...
y la guía de estilo entera— para que quien escriba no tenga que adivinar nada.
    ...
    python3 web-es/tools/siguiente-nota.py            briefing en texto
    python3 web-es/tools/siguiente-nota.py --json     lo mismo, para scripts
    python3 web-es/tools/siguiente-nota.py --estado   qué se ha publicado y qué falta

Códigos de salida: 0 hay tema · 3 se han acabado los temas pendientes.

El estado real lo manda el disco, no el JSON: un tema está publicado si existe
su archivo en web-es/contenido/blog/. El campo «estado» del calendario se
sincroniza solo en cada ejecución, así que si la rutina se cae después de
escribir el .md pero antes de hacer el commit, la siguiente no repite el tema.
"""
import json
import os
import re
...
def main():
    ...
    if '--estado' in args:
        ...
        if not faltan:
            print('\nNo quedan temas. Añade más en web-es/contenido/calendario.json.')
            return SIN_TEMAS
        return 0
    ...
    if tema is None:
        aviso = ('SIN TEMAS PENDIENTES.\n'
                 'El calendario editorial (web-es/contenido/calendario.json) se ha quedado sin '
                 'temas con estado «pendiente». No inventes uno: avisa y no publiques nada.')
        if '--json' in args:
