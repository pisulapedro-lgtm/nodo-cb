# -*- coding: utf-8 -*-
"""Arma una vista previa navegable de climabaires.es en un solo archivo HTML.

Uso: python3 web-es/tools/preview-html.py
...
SALIDA = os.path.normpath(os.path.join(RAIZ, '..', 'dist', 'preview-climabaires-es.html'))
SALIDA_MOVIL = os.path.normpath(os.path.join(RAIZ, '..', 'dist', 'preview-movil-climabaires-es.html'))

# Las fotos se recomprimen para la vista previa: a 640 px se ven bien en el marco
# de móvil y el archivo entero baja de 4 MB a menos de 1.
ANCHO_PREVIEW = 640
CALIDAD_PREVIEW = 70
...
GRUPOS = [
    ...
    ('Blog', []),
    ('Legales', ['aviso-legal.html', 'privacidad.html', 'cookies.html', 'terminos.html', '404.html']),
]
...
DOC = """<title>climabaires.es — vista previa navegable</title>
<style>
%(css_shell)s
</style>
...
<header class="barra">
  ...
  <div class="anchos" role="group" aria-label="Ancho de pantalla">
    <button class="ancho activo" data-w="390">Móvil</button>
...
DOC_MOVIL = """<title>climabaires.es</title>
...
<div class="pantalla">
  <iframe id="vista" title="climabaires.es"></iframe>
</div>
...
# En el móvil el sitio va a sangre, sin nada alrededor: es la simulación.
# En una pantalla ancha se centra una columna del ancho de un móvil, porque si no
# el navegador rendiría la versión de escritorio y dejaría de simular justamente
...
/* de 760 px para arriba ya no es un móvil: se acota a un móvil centrado */
