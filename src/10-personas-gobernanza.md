# 10. Personas y gobernanza

## 10.1 Quién hace qué en el mes 1

| Rol | Quién | Alcance |
|---|---|---|
| CEO — todos los roles en uno | **Ignacio** (Buenos Aires) | Comercial, administración, contabilidad de gestión, dirección, control de calidad de instalaciones. **No instala.** |
| Instalación | **2 monotributistas subcontratados por instalación** | Locación de instalación por instalación, con herramientas y vehículo propios |
| Marketing digital y web | **Pedro** (remoto, Málaga) | Kit digital, contenidos, campañas, seguimiento de la web |
| Finanzas, pacto y seguros | **Sebastián** (remoto, Málaga) | Tablero de caja, seguros, coordinación del pacto y del SWIFT |
| Contador matriculado (externo) | A contratar día 1-3 | Inscripciones, liquidaciones, padrones — la firma fiscal no puede ser de Ignacio aunque lleve los números |
| Escribano + abogado societario (externos) | A contratar semana 1 | Constitución, poderes, pacto, contratos de locación de obra |

## 10.2 Dependencia vs. monotributista — la decisión y sus riesgos

**Fase 1 (agosto-noviembre): subcontratación pura.** El marco 2026 es el más favorable en décadas — la Ley 27.742 derogó las multas por empleo no registrado y el nuevo art. 23 LCT (Ley 27.802, vigente 03-2026) excluye la presunción de laboralidad en contrataciones de instalación con factura y pago bancarizado [F39]. **Pero la primacía de la realidad subsiste**: si un juez ve horario fijo, órdenes permanentes, exclusividad y herramientas nuestras, condena igual (indemnización + diferencias del convenio UOM retroactivas + cargas con intereses).

**Checklist antifraude (auditar cada trimestre, sin excepciones):**
- Contrato de locación de obra **por instalación** (nunca «servicios mensuales»), con precio por instalación y derecho del instalador a rechazar instalaciones y hacerse sustituir.
- Factura por instalación de montos variables + pago 100% bancarizado (condición del art. 23 LCT).
- Evidencia archivada de que el instalador tiene **otros clientes**; exclusividad prohibida — por contrato y de hecho.
- Herramientas y vehículo propios del instalador (declarados en el contrato con fotos en el legajo).
- Sin horario, sin uniforme obligatorio, sin sanciones, sin «presente» en grupos de WhatsApp.
- Póliza AP vigente con no repetición + constancia de monotributo del mes, **antes** de cada instalación.
- Ignacio jamás factura como monotributista a la propia SAS: su retribución es honorario de gerente y dividendos. [VALIDAR CON CONTADOR]

**Umbral de conversión**: un instalador con >15 instalaciones/mes solo para nosotros durante 3+ meses es dependencia económica de hecho → pasarlo a plantilla. [VALIDAR CON ABOGADO]

## 10.3 Costo laboral total por puesto (para decidir con números)

| Puesto | Esquema | Costo mensual total | Cuándo |
|---|---|---|---|
| Instalador subcontratado | Locación de instalación + AP | ≈ $ 120.000-180.000 **por instalación** † (según complejidad; tarifario CAR [F36]) + AP $ 15-20.000/mes si la paga la empresa [F24] | Desde el día 1 |
| Técnico en dependencia (UOM Rama 17) | Bruto $ 1.500.000 × coef. ≈ 1,46 † | **≈ $ 2.183.000/mes (€ 1.240)** [F13] | Diciembre, si el pipeline > 15 instalaciones/mes |
| Administrativo/comercial (CCT 130/75) | Bruto $ 1.350.000-1.500.000 | ≈ $ 1.950.000-2.100.000/mes [F44] | Fase 2 (2027), cuando Ignacio no dé abasto |
| Ignacio (gerente) | Autónomos obligatorios + honorarios | Autónomos ≈ $ 141.000/mes [F14]; **sin sueldo del capital en 2026** | — |

## 10.4 KPIs semanales de Ignacio a los socios (tablero compartido, 20 minutos de carga)

| KPI | Meta sep | Meta nov |
|---|---|---|
| Leads nuevos (WhatsApp + web + Ads) | 20-30/mes | 80-110/mes |
| Tasa de respuesta < 5 min en horario comercial | 90% | 95% |
| Presupuestos enviados / leads | > 60% | > 70% |
| Cierre: instalaciones vendidas / presupuestos | 25-30% | 25-30% |
| Instalaciones terminadas (con checklist firmado) | 4-8 | 18-28 |
| Ticket promedio (equipo + instalación) | $ 1,8-2,5M | $ 2-2,5M |
| Margen bruto por instalación | ≥ 30% | ≥ 35% |
| Caja disponible / meses de fijos cubiertos | ≥ 5 | ≥ 4 |
| Reseñas Google acumuladas | 5 | 15-20 |

## 10.5 Cadencia España–Argentina

- **Lunes 18:00 AR / 22:00 ES** (30 min): videollamada semanal de socios — KPIs, caja, bloqueos, decisiones de la semana. Sin agenda escrita previa no hay reunión.
- **Diario asincrónico**: grupo de socios en WhatsApp + tablero compartido siempre actualizado (Sheet de KPIs + Drive de instalaciones). Regla: los números se miran en el tablero, no se preguntan por chat.
- **Mensual** (1 h): revisión presupuesto vs. real + decisión de escalado (pauta, contratación, stock de temporada).
- **Decisiones reservadas al pacto** (mayoría 75%): las listadas en la sección 2 — el día a día es 100% de Ignacio.

## 10.6 Gobernanza del pacto (lo esencial)

Mayoría de Ignacio (40%) **condicionada a su permanencia como gerente**; vetos de minoría para decisiones estructurales; información mensual obligatoria (el tablero); no competencia y dedicación pactadas; resolución de conflictos por mediación y, si falla, arbitraje; cláusulas de buen/mal saliente sobre las participaciones. El term sheet se firma en Semana 0 y se eleva con el aumento de capital de octubre. [VALIDAR CON ABOGADO]
