...
| `build/` | Pipeline del PDF (`build.mjs`, plantilla CSS A4, assets de marca), los motores de cálculo (`financiero.mjs` y `marketing.mjs`), el generador de la planilla (`modelo-excel.py`) y el de miniaturas (`preparar-miniaturas.py`) |
...
El PDF reúne todo el proyecto en cinco partes:
...
| — | 20 | Anexos: glosario, checklist de España y las fuentes fechadas |
...
npm run build        # → dist/Plan-de-Negocio-Clima-Baires-Argentina_v1.2.pdf
npm run modelo       # → dist/Modelo-Financiero-Clima-Baires-Argentina_v1.2.xlsx
npm run build -- --version 1.3   # para emitir una versión distinta de la de datos.json
```
...
- **v1.2** — suma la Parte V: plan de marketing y publicidad con el embudo cuantificado y las campañas de Google Ads y Meta listas para cargar. 20 secciones, 47 fuentes. Agosto 2026.
...
Pendiente de los socios: revisión del v1.2, el **número real de WhatsApp Business** (hoy hay un placeholder en `web/assets/js/main.js` y sin él la web no se publica) y la validación profesional de los puntos marcados `[VALIDAR CON …]`.
